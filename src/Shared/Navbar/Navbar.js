                ...
                <nav class=" max-w-7xl mx-auto lg:flex lg:justify-between lg:items-center">
                    ...
                    <div className={`absolute inset-x-0 z-20 w-full px-6 py-4 transition-all duration-300 ease-in-out bg-white uppercase  md:bg-transparent md:dark:bg-transparent md:mt-0 md:p-0 md:top-0 md:relative md:w-auto md:opacity-100 md:translate-x-0 md:flex md:items-center ${isOpen ? 'translate-x-0 opacity-100 ' : 'opacity-0 -translate-x-full'}`}>
                        <div class="flex flex-col space-y-4 lg:mt-0 lg:flex-row lg:space-y-0 font-semibold">
                            <Link onClick={() => setOpen(!isOpen)} to='/' class="text-gray-700 lg:mx-6 dark:text-gray-200 dark:hover:text-blue-400 hover:text-blue-500" >Home</Link>
                            <Link onClick={() => setOpen(!isOpen)} class="text-gray-700 lg:mx-6 dark:text-gray-200 dark:hover:text-blue-400 hover:text-blue-500" to='/aboutus'>About Us</Link>
                            <Link onClick={() => setOpen(!isOpen)} class="text-gray-700 lg:mx-6 dark:text-gray-200 dark:hover:text-blue-400 hover:text-blue-500" to='/advice'>Advice For You</Link>
                            <Link onClick={() => setOpen(!isOpen)}  to='/contact' class="text-gray-700 lg:mx-6 dark:text-gray-200 dark:hover:text-blue-400 hover:text-blue-500">Contact</Link>
                            <Link onClick={() => setOpen(!isOpen)} to='/carrer' class="text-gray-700 lg:mx-6 dark:text-gray-200 dark:hover:text-blue-400 hover:text-blue-500">Carrer</Link>
                        </div>

                        <Link onClick={() => setOpen(!isOpen)} to='/allcourses' class="block h-10 px-5 py-2 mt-4 text-sm text-center outline-none border-none capitalize transition-colors duration-300 transform border rounded-md dark:hover:bg-gray-700 dark:text-white lg:mt-0 hover:bg-gray-100 lg:w-auto background-radial-gradient text-white font-semibold " >
                            Our Courses
                        </Link>
                    </div>
                </nav >
